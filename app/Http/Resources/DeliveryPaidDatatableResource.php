...
class DeliveryPaidDatatableResource extends JsonResource
{
    /**
    ...
    public function toArray($request)
    {
        ...
        if (hasPermission('delivery_paid', 'write')) {
            $print = "<a class='btn btn-default btn-sm' style='border: 1px solid #227fc1;'
                         ...
                         target='_blank' href='" . url('delivery-paid/print', $this->id) . "'>
                         <i class='far fa-print text-primary'></i>
                     </a>&nbsp;";
        }

        $delete = '';
        if (hasPermission('delivery_paid', 'delete')) {
            $delete = Form::open(['route' => ['delivery-paids.destroy', $this->id], 'method' => 'delete', 'class' => 'd-inline'])
