...
class NeedDelDoneDatatableResource extends JsonResource
{
    /**
    ...
    public function toArray($request)
    {
        ...
        if (hasPermission('need_deneed_del_donelivery', 'write')) {
            $print = "<a class='btn btn-default btn-sm' style='border: 1px solid #227fc1;'
                         data-toggle='tooltip' data-placement='top'  data-title='Print Barcode'
                          target='_blank' href='" . url('need-del-done/print', $this->id) . "'>
                         <i class='far fa-print text-primary'></i>
                     </a>&nbsp;";
        }

        $delete = '';
        if (hasPermission('need_del_done', 'delete')) {
            $delete = Form::open(['route' => ['need-del-dones.destroy', $this->id], 'method' => 'delete', 'class' => 'd-inline'])
        ...
        return [
            ...
            'total' => '$ ' . number_format($this->total ?  $this->total : $price, 2),
