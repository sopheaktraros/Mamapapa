...
class StockPendingPaymentDatatableResource extends JsonResource
{
    /**
    ...
    {
        $show = "";
        if(hasPermission('stock_pending_payment', 'read')){
            $show = "<a href='" . route('stock-pending-payments.show', $this->id) . "'
        ...
        $status = "<small data-title='$this->remark' data-toggle='tooltip' data-placement='left' 
                class='text-white p-1 " . ($this->status == 2 ? "bg-primary" : "bg-success") . "'>"
                . ( $this->status == 2 ? "Pending Paymnet" : "Done")
                . "</small>";
