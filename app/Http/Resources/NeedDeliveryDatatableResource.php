...
class NeedDeliveryDatatableResource extends JsonResource
{
    /**
        ...
        if (hasPermission('need_del_pending', 'write')) {
            $print = "<a class='btn btn-default btn-sm' style='border: 1px solid #227fc1;'
                         data-toggle='tooltip' data-placement='top'  data-title='Print Barcode'
                          target='_blank' href='" . url('need-deliveries/print', $this->id) . "'>
                         <i class='far fa-print text-primary'></i>
                     </a>&nbsp;";
        }

        $delete = '';
        if (hasPermission('need_del_pending', 'delete')) {
            $delete = Form::open(['route' => ['need-deliveries.destroy', $this->id], 'method' => 'delete', 'class' => 'd-inline'])
