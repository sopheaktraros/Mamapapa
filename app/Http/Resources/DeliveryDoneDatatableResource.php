...
class DeliveryDoneDatatableResource extends JsonResource
{
    /**
    ...
    public function toArray($request)
    {
        ...
        if (hasPermission('delivery_done', 'write')) {
            $print = "<a class='btn btn-default btn-sm' style='border: 1px solid #227fc1;'
                         ...
                         target='_blank' href='" . url('delivery-done/print', $this->id) . "'>
                         <i class='far fa-print text-primary'></i>
                     </a>&nbsp;";
        }

        $delete = '';
        if (hasPermission('delivery_done', 'delete')) {
            $delete = Form::open(['route' => ['delivery-dones.destroy', $this->id], 'method' => 'delete', 'class' => 'd-inline'])
