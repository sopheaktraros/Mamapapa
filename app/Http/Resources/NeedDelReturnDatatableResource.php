...
class NeedDelReturnDatatableResource extends JsonResource
{
    /**
        ...
        if (hasPermission('need_del_return', 'write')) {
            $print = "<a class='btn btn-default btn-sm' style='border: 1px solid #227fc1;'
                         data-toggle='tooltip' data-placement='top'  data-title='Print Barcode'
                          target='_blank' href='" . url('need-del-return/print', $this->id) . "'>
                         <i class='far fa-print text-primary'></i>
                     </a>&nbsp;";
        }

        $delete = '';
        if (hasPermission('need_del_return', 'delete')) {
            $delete = Form::open(['route' => ['need-del-returns.destroy', $this->id], 'method' => 'delete', 'class' => 'd-inline'])
