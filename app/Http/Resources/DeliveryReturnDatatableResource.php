...
class DeliveryReturnDatatableResource extends JsonResource
{
    /**
    ...
    public function toArray($request)
    {
        ...
        if (hasPermission('delivery_return', 'write')) {
            $print = "<a class='btn btn-default btn-sm' style='border: 1px solid #227fc1;'
                         ...
                         target='_blank' href='" . url('delivery-return/print', $this->id) . "'>
                         <i class='far fa-print text-primary'></i>
                     </a>&nbsp;";
        }

        $delete = '';
        if (hasPermission('delivery_return', 'delete')) {
            $delete = Form::open(['route' => ['delivery-returns.destroy', $this->id], 'method' => 'delete', 'class' => 'd-inline'])
        ...
        if(hasPermission('delivery_return', 'write')){
            $checkbox = '<div class="custom-control custom-checkbox" style="margin-left:5px">
                            <input data-id="' . $this->id . '" id="' . $this->id . '" type="checkbox" class="custom-control-input delivery-check" name="delivery_checked[' . $this->id . ']" value="' . $this->id . '">
        ...
        if(hasPermission('delivery_return', 'write')){
            ...
            $inputDeliveryman = '<input readonly=' . $readonly . ' type="text" name="deliveryman_code['. $this->id .']" class="form-control-sm" value=' . $this->delivery_man_code . '>';
        }
